...
module.exports = async function (req: Request, res: Response) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id_horario, bloque_comida, hora_inicio, hora_fin FROM Horario_Restaurante WHERE id_restaurante = ?`,
      [restauranteId]
    );
    await pool.end();
    res.status(200).json(rows);
  } catch (error) {
    res.status(500).json({ error: "Error al obtener horarios", details: (error as any).message });
  }
}
