...
module.exports = async function (req: Request, res: Response) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT r.id_reserva, r.fecha_reserva, r.num_personas, r.alergias, r.estado, r.confirmacion,
    ...
    res.status(500).json({ error: "Error al obtener reservas", details: (error as any).message });
  }
}
